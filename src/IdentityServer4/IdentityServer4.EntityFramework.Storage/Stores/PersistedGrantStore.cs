...
namespace IdentityServer4.EntityFramework.Stores
{
    ...
    public class PersistedGrantStore : IPersistedGrantStore
    {
        ...
        public virtual async Task StoreAsync(PersistedGrant token)
        {
            ...
            // todo check limited sessionId allowed with SubjectId in ClientId
            
            // end
            //if (checkLimited)
            //{
            //    throw new DbUpdateConcurrencyException("Reached limited sessionId allowed with SubjectId in ClientId"); 
            //}
        ...
        public async Task<bool> CheckLimitedAsync(string subjectId, string grantType, string clientId)
        {
            var checkLimited = ((await Context.PersistedGrants
                                    .Where(x => x.Type == grantType && x.SubjectId == subjectId && x.ClientId == clientId).ToArrayAsync()).Count() >= 2) ? true : false;
            //var xx = await Context.PersistedGrants.Where(x => x.Type == grant.Type && x.SubjectId == grant.SubjectId && x.ClientId == grant.ClientId).ToArrayAsync();
            return checkLimited;
        }
    }
}